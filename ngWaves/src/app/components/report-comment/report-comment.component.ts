...
export class ReportCommentComponent implements OnInit {
  ...
  addReportComment(rComment: ReportComment) {
    ...
    this.reportCommentService.create(rComment).subscribe(
      (created) => {
        console.log(
          'ReportCommentListComponenet.addReportComment(): created Report Comment'
        );
        console.log(created);
        // this.loadReportComment();
      },
      (failed) => {
        console.error(
          'ReportCommentListComponenet.addReportComment(): Error Creating Report Comment'
        );
      }
    );
    // this.loadReportComment();
  ...
  authCheck() {
    let login = this.authService.checkLogin();
    console.log(login);
  }
}
